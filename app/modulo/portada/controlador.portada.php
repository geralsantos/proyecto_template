...
class portada extends App{
    ...
    public function buildTreeHtml_2($elements,$opt=""){
      ...
      foreach ($elements as $element){
          ...
          if(isset($element['children'])){
            ...
            if($element['admin']!=1){
              $li = $li  .  '<li >
              <a data-toggle="collapse" data-target="#' . $element['id'] .'"  >' . $element['nombre'] .'</a>
                <ul id="' . $element['id'] .'" class="collapse dropdown-header-top">
                    '. $this->buildTreeHtml($element['children'],'childs').'
                </ul>
                </li> ';
            }else{
    ...
    public function cargar_jugadores_subastas($callback=false){
      ...
      $jugador_id=isset($_POST["jugador_id"])? " and js.jugadores_id='".$_POST["jugador_id"]."' ":"";
      $subastas_ligas = isset($_POST["subastas_liga"])? " lig.usuario_id='$idusuario' $jugador_id": " lp.usuario_id='$idusuario' " ; //para subastas-liga.php
      ...
      if (isset($_POST["subastas_liga"])) {
        ...
        $groupby = "group by js.jugadores_id,lp.liga_id";//para subastas-liga.php
      }
      $query = "SELECT jug.*,$case, lp.id as liga_participantes_id, lp.liga_id, usu.id as usu_id, usu.nombre as usu_nombre,usu.apellido as usu_apellido, usu.id_fb, usu.correo as usu_correo, usu.usuario as usu_usuario, usu.clave, js.jugadores_id, jugval.valor as precio_base_jugador FROM  jugadores_subastas js
    ...
    public function cargar_jugadores_compra(){
      ...
      if( $_POST['datos_liga'] &&  $_POST["team"]  ){
        $modelo = new modeloPortada();
        return $modelo->cargar_jugadores_compra( $_POST['team'], $_POST['datos_liga']);
      }
    ...
    public function configuracion_equipo(){
      ...
      if( $_POST["equipo_id"] && $_POST["equipo_nombre"] ){
        ...
        $arrData = array("nombre"=>$_POST["equipo_nombre"], "usuario_edicion"=> $_SESSION["usuario"][0]["id"],"fecha_edicion"=> date("Y-m-d H:i:s"));
        if (move_uploaded_file($tmp_archivo, $fichero_subido)){
          $arrData["avatar"]=$nombre_archivo_bd;
        }
        $modelo = new modeloPortada();
        $res = $modelo->updateData('equipos',$arrData, array("id"=>$_POST["equipo_id"]));
        if($res){
          ...
          if ($update) { //actualizando al usuario
            ...
            if ($res) {
              if (count($valores_new_usuario)==1) {
                $jugadores_subastas = $modelo->executeQuery("SELECT *,usu.nombre as usuario_nombre,usu.apellido as usuario_ape FROM jugadores_subastas js inner join liga_participantes lp on(js.liga_participantes_id=lp.id) inner join usuarios usu on (lp.usuario_id=usu.id)
                ...
                echo json_encode(array("resultado"=>true,"ganador_default"=>true,"mensaje"=>("Ganador(a) de la subasta por que todos se retiraron: ".$jugadores_subastas[0]["usuario_nombre"]." ".$jugadores_subastas[0]["usuario_ape"]),"data"=>$jugadores_subastas));
  ...
  public function renovar_plan_culqi(){
    if(isset($_SESSION["usuario"])){
      if ($this->inscripcion_pago(true)) {
        $modelo = new modeloPortada();
        $plan_id=$_REQUEST["id_plan"];
        $id_usuario=$_SESSION["usuario"][0]["id"];
        $dataupdate=array("plan_id"=>$plan_id,"activo"=>1);
        $where=array("id"=>$id_usuario,"estado"=>1);
        $update = $modelo->updateData("usuarios", $dataupdate, $where);
        if ($update) {
          echo json_encode(array("resultado"=>true));
        }else{
          echo json_encode(array("resultado"=>false));
    ...
    public function cargar_usuario(){
      $modelo = new modeloPortada();
      $res = $modelo->selectRowData('usuarios','*', array("id"=>$_SESSION["usuario"][0]["id"]) );
      if($res){
